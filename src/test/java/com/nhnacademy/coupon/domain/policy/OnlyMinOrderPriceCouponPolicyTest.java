...
class OnlyMinOrderPriceCouponPolicyTest {
    ...
    void testGetMaxDiscountPrice() {
        CouponPolicy policy = new OnlyMinOrderPriceCouponPolicy(1L,200L,1000L,0.20D,CouponDisCountType.RATE);
    ...
    void testGetMaxDiscountPrice1() {
        CouponPolicy policy = new OnlyMinOrderPriceCouponPolicy(1L,100L,1000L,0.20D,CouponDisCountType.RATE);
        Assertions.assertThat(policy.getSalePrice(new Price(100L)).value()).isEqualTo(20L);
    }
    @Test
    @DisplayName("최대 할인금액은 없고, 최소할인금액이 있으면 작동한다.")
    void test3() {
        CouponPolicy policy = new OnlyMinOrderPriceCouponPolicy(1L,100L,null,0.1D,CouponDisCountType.RATE);
